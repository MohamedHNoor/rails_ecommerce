class AdminController < ApplicationController
 ...
 def index
  ...
  @order_by_day = Order.where('created_at > ?', Time.now - 7.days).order(:created_at)
  @order_by_day = @order_by_day.group_by { |order| order.created_at.to_date }
  @revenue_by_day = @order_by_day.map { |day, orders| [day.strftime("%A"), orders.sum(&:total)]}
